    ...
    public String kill(String killId, String key, Integer num) throws InterruptedException {
        ...
        if(currentTime >=startTime && currentTime<= endTime){
            ...
            if(randomCode.equals(key) && skuId.equals(killId)){
                ...
                //获取信号量
                String seckillCount  = redisTemplate.boundValueOps(SKU_STOCK_SEMAPHORE + randomCode).get();
                ...
                if (count > 0 && num <= seckillLimit && count > num ) {
                    ...
                    if(aBoolean){
                        ...
                        //TODO 秒杀成功，快速下单
                        boolean semaphoreCount  = semaphore.tryAcquire(num, 100, TimeUnit.MINUTES);
                        //保证Redis中还有商品库存
                        if (semaphoreCount) {
                            //创建订单号和订单信息发送给MQ
